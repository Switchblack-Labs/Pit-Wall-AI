...
def test_explain_never_500s_without_strategy(client):
    # With no strategy set yet, the endpoint returns a structured response, not 500.
...
def test_torcs_start_and_stop(client):
    ...
    # Immediately stop so no background loop leaks into other tests.
...
def test_websocket_connect_and_receive_broadcast(client):
    # Connect, then trigger a strategy broadcast and confirm it arrives.
