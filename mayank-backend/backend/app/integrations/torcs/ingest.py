...
class TorcsIngestor:
    ...
    async def ingest_frame(self, frame: TorcsTelemetry) -> None:
        # 1. Core telemetry via the existing adapter + service.
        ...
        # 3. Richer TORCS-only fields (sector times, race events) over a
        #    dedicated channel so existing "telemetry" consumers are unaffected.
